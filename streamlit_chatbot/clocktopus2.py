...
with tabs[1]:
    ...
    st.write(f"**{city1}:** {time1.strftime('%Y-%m-%d %H:%M:%S')}")
    st.write(f"**{city2}:** {time2.strftime('%Y-%m-%d %H:%M:%S')}")
...
with tabs[2]:
    ...
    st.info("Expand this with live APIs for extra wow!")

# -------------------- EARTH FACTS --------------------
with tabs[3]:
    st.subheader("🌏 Earth Facts")
    st.write("""
...
# -------------------- WORLD WONDERS --------------------
with tabs[4]:
    st.subheader("🧭 7 Wonders of the World (Emoji Edition)")
    wonders = [
    ...
    for emoji, place in wonders:
        st.write(f"{emoji} — {place}")

# -------------------- SPECIAL TENTACLES --------------------
with tabs[5]:
    st.subheader("🧩 Special Tentacles")
    st.write("""
    🐙 Did you know? An octopus has **3 hearts** and **blue blood**!

    🧠 Each of its **8 tentacles** can **move independently** because they have **mini-brains** inside!

    🌊 An octopus can **camouflage** instantly by changing its skin **color** and **texture** — it’s a living, breathing shapeshifter.

    🧬 Some octopuses can **regrow lost arms** — chop it off and it just grows back!

    💧 They squeeze their entire squishy body through holes no bigger than their **beak**.

    🕵️‍♂️ They’re escape artists — an octopus once broke out of its tank, crawled across the floor, and vanished into the sea!

    🎨 They’re some of the **smartest invertebrates** — solving puzzles, opening jars, even playing with toys.

    🌍 Just like **Clocktopus**, every tentacle reaches into a different part of the world!

    🧩 Next time you see an octopus, remember: it’s basically an underwater alien!
    """)
